...
function printInformation(hideWork,index)
{
	...
	//jQuery(idSlide + ' img').load(function(){//esperar a que la primera imagen se cargue
	generateSlider(idSlide, idSlideNext, idSlidePrev);	
	//});
 	/*spinner.stop();
	jQuery.unblockUI();	*/
	hideSpinner();
...
function filterAll()
{
	...
	jQuery( '.buttonPort' ).css("background-position-y", "top");
